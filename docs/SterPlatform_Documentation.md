...
| 14 | Mercure — `MERCURE_JWT_SECRET` trop court | `lcobucci/jwt` exige un secret HMAC-SHA256 d'au moins **256 bits (32 caractères)**. Avec `sterplatform_mercure_secret` (27 chars = 216 bits), le bundle lève `InvalidKeyProvided: "Key provided is shorter than 256 bits"` à chaque appel `GET /api/mercure/token`. L'erreur se reproduit en CI si la variable n'est pas explicitement injectée dans le workflow. | Utiliser un secret d'au moins 32 caractères partout : `.env`, `.env.test`, `docker-compose.yml` (fallback), et **workflow CI** (`env:` au niveau job). Ne pas se fier au chargement implicite du `.env` en CI — toujours déclarer `MERCURE_JWT_SECRET` explicitement dans `tests.yml`. |
