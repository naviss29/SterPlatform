...
| 25 | Mai 2026 | Phase 5b — Auth générique étendu | `AuthController` : `redirect_uri` query param sur `GET /api/auth/verify` → `RedirectResponse` au lieu de JSON (backwards-compatible). `register` accepte `verifyRedirectUri` (body) → lien email contient `&redirect_uri=...`. `forgotPassword` accepte `resetRedirectBaseUrl` (body) → lien email utilise ce base URL au lieu de `APP_URL`. `MailerService` : `sendVerificationEmail(?string $redirectUri)` + `sendPasswordResetEmail(?string $resetBaseUrl)`. 1 test ajouté `testVerifyWithRedirectUri()` → 46/46 passants. |
| 26 | Mai 2026 | Nettoyage architectural | Suppression de tout le code DartsOpen-spécifique introduit en Phase 5a/5c/5d : 8 enums, 7 entités Doctrine, 7 repositories, 4 controllers (TournamentController, PoolController, MatchController, MatchSetController), migration `Version20260515210041`. Revert MercurePublisher (suppression `publishWithTournamentTopic`) et DoctrinePublishSubscriber (suppression `extractTournamentId`). SterPlatform redevenu infra générique pure : auth JWT + email + Mercure + multi-tenancy. 46/46 tests passants. |
...
- [x] Phase 5b — Auth générique étendu (redirect_uri sur verify, verifyRedirectUri/resetRedirectBaseUrl — 46 tests)
- [x] Nettoyage architectural — suppression de tout le code DartsOpen-spécifique (Phase 5a/5c/5d revertis), SterPlatform = infra pure
- [ ] Phase 6 — Production multi-projets (guide intégration pour copains devs, versioning API, OpenAPI publique)
