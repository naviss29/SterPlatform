...
> Auteur : Alan
> Date : Mai 2026
> Statut : **Production opérationnelle — https://sterplatform.bichetapps.com — Phase 5c terminée (migration DartsOpen complète, Supabase supprimé hors Stripe)**
...
| 27 | Mai 2026 | Phase 5c — Migration DartsOpen vers SterPlatform | **SterPlatform** — Sérializers standardisés snake_case : `TournamentController` (max_players, nb_pools, nb_boards, entry_fee, created_at…), `PoolController` (player_name, board_number, player1_id, player2_id, pool_id), `MatchController` (board_number, bracket_round, bracket_position, validated_p1, validated_p2). `registered_count` ajouté à `GET /api/public/tournaments/{id}` (calcul `findPaidByTournament`). **DartsOpen** — 7 pages dashboard réécrites : liste tournois, nouveau tournoi, détail, activate, players, pools, bracket. 3 pages publiques : register (registered_count), score (sets→match_sets mapping), live. 4 composants nettoyés : ScoreForm (tournamentId prop), MatchBoard (polling 3s), ScoreBoard (polling 5s), BracketLive (polling 5s). `lib/actions/bracket.ts` : generateBracket + advanceToNextRound via SterPlatform (auto-advance via tryAdvanceBracket). `lib/api/tournament.ts` : apiListPoolsPublic ajouté. 1314 lignes supprimées, 792 ajoutées — Supabase supprimé de toutes les pages/composants hors Stripe. |
| 26 | Mai 2026 | Phase 5b — Auth DartsOpen | `AuthController` : `redirect_uri` query param sur `GET /api/auth/verify` → `RedirectResponse` au lieu de JSON (backwards-compatible). `register` accepte `verifyRedirectUri` (body) → lien email contient `&redirect_uri=...`. `forgotPassword` accepte `resetRedirectBaseUrl` (body) → lien email utilise ce base URL au lieu de `APP_URL`. `MailerService` : `sendVerificationEmail(?string $redirectUri)` + `sendPasswordResetEmail(?string $resetBaseUrl)`. 1 test ajouté `testVerifyWithRedirectUri()` → 46/46 passants. **DartsOpen** : `lib/api/client.ts` (base fetch + `X-Organization-Slug` header), `lib/api/auth.ts` (`getUser`, `setAuthCookies`, `clearAuthCookies`, `tryRefresh` auto-refresh JWT), `lib/actions/auth.ts` réécriture complète (fetch SterPlatform), `middleware.ts` (protection `/dashboard`, `/tournaments`, `/settings` via cookie `ster_token`), `app/(dashboard)/layout.tsx` utilise `getUser()`, `app/(auth)/reset-password/page.tsx` lit `token` depuis `searchParams`, `ResetPasswordForm` accepte prop `token` + champ hidden, `app/auth/verified/page.tsx` (page confirmation email), `app/auth/callback` + `app/auth/confirm` simplifiés. |
...
- [x] Phase 5b — Auth DartsOpen (lib/api/client + auth, actions auth, middleware, dashboard layout — 46 tests SterPlatform)
- [x] Phase 5c — API calls DartsOpen (sérializers snake_case, 10 pages + 4 composants migrés, polling REST, Supabase supprimé hors Stripe)
- [ ] Phase 5d — Real-time DartsOpen (remplacer polling setInterval par EventSource Mercure SSE)
